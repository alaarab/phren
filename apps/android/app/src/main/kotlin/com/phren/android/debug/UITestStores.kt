...
/**
 * The iOS UI-test stores (UITestFixtures.swift populateTour/populateMemory),
 * so Android screens can be compared with the iOS captures side by side.
 * Generated from the Swift source; debug launches only.
 */
object UITestStores {
    private fun finding(id: String, text: String) = "- $text <!-- fid:$id --> <!-- created: 2026-09-14 --> <!-- phren:status \"active\" -->\n"
    private fun finding(id: String, date: String, text: String) = "- $text <!-- fid:$id --> <!-- created: $date --> <!-- phren:status \"active\" -->\n"
    fun populateTour(store: LocalStore) {

        store.write("phren/FINDINGS.md", "# phren Findings\n\n## 2026-09-16\n\n"
            ...
            + finding("e4f5a6b7", "[pattern] Controls stay at least 44 points; reduce padding and duplicate rows before text size."), null)
        store.write("phren/tasks.md", """
        # phren tasks
...
""".trimIndent(), null)
        store.write("phren/phren.project.yaml", "ownership: repo-managed\nsourcePath: /work/phren\n", null)
        store.write("phren/skills/design.md", SkillFile.template(name = "design", description = "Design pass on a named screen", instructions = "Fix hierarchy, spacing and density against phren's own conventions, then verify on the simulator."), null)
        store.write("mina/FINDINGS.md", "# mina Findings\n\n## 2026-09-14\n\n"
            ...
            + finding("a6b7c8d9", "[pattern] Seed the simulator with twenty days of demo data before any screenshot run."), null)
        store.write("mina/tasks.md", """
        # mina tasks
...
""".trimIndent(), null)
        store.write("mina/phren.project.yaml", "ownership: repo-managed\nsourcePath: /work/mina\n", null)
        store.write("atlas/FINDINGS.md", "# atlas Findings\n\n## 2026-09-11\n\n"
            ...
            + finding("a3b4c5d6", "[pitfall] Tiles are one sprite sheet: a missing tile is a wrong offset, not a missing file."), null)
        store.write("atlas/phren.project.yaml", "ownership: repo-managed\nsourcePath: /work/atlas\n", null)
    }

    fun populateMemory(store: LocalStore) {
        ...
        store.write("phren/FINDINGS.md", "# phren Findings\n\n## 2026-09-19\n\n"
            ...
            + finding("f7e8d9c0", "2026-09-10", "[pattern] Access tokens live for 10 minutes and refresh tokens for 30 days; refresh rotation revokes the old token on first use to detect replay."), null)
        store.write("phren/tasks.md", """
        # phren tasks
...
""".trimIndent(), null)
        store.write("phren/notes/2026-09-18.md", """
        ## 09:12 <!-- nid:aa11bb22 -->
...
""".trimIndent(), null)
        store.write("ledger/FINDINGS.md", "# ledger Findings\n\n## 2026-09-16\n\n"
            ...
            + finding("1f3a4b5c", "2026-09-08", "[workaround] The export job streams CSV rows instead of building the file in memory; the 2GB export no longer restarts the worker."), null)
        store.write("ledger/tasks.md", """
        # ledger tasks
