...
- "Hey Siri, talk to my conductor" (also the Action button, Shortcuts, and a new Talk to Conductor control for Control Center and the lock screen) opens your running conductor's chat with talk mode on. The conversation keeps going with the screen locked and the phone in a pocket, through AirPods too: it listens until you pause, sends, reads the reply aloud and listens again, and talking over a reply stops it. With no conductor running, it opens Start a conductor instead. The control starts talk mode with a tap; a control can't be held down. This phrase replaces "Tell my conductor", which is still in the Shortcuts app.
- A Pause All Agents control for Control Center and the lock screen opens phren on a confirmation that names every working agent on your computers. Only Pause interrupts them (the same stop as the chat's stop button: they stay open and wait for your next message), and phren then says how many paused and which couldn't be reached.
