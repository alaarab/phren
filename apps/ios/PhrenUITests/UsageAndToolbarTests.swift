...
final class UsageAndToolbarTests: XCTestCase {
    ...
    func testUsageShowsBothProvidersPercentagesAndResetTimes() {
        ...
        XCTAssertTrue(app.staticTexts["$1.20 of $2.00"].exists)
        XCTAssertTrue(app.staticTexts["$4.80 of $5.00"].exists)
        XCTAssertTrue(app.staticTexts["$9.10 of $10.00"].exists)
        ...
        XCTAssertTrue(app.staticTexts["7-day, Fable only"].exists)
        XCTAssertTrue(app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH %@", "resets in ")).firstMatch.exists)
        XCTAssertTrue(app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH %@", "from Claude Code status line, updated ")).firstMatch.exists)
