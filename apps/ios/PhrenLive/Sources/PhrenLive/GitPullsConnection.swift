...
extension PhrenConnection {
    ...
    /// or not signed in.
    public static func gitPulls(host: LiveHost, privateKey: Data, target: AgentChatTarget, child: String? = nil) async throws -> GitPulls {
        try gitPullsTarget(host, target)
        let request = try gitPullsRequest(target: target, child: child)
    ...
    private static func gitPullsRequest(target: AgentChatTarget, child: String?) throws -> GatewayRequest {
