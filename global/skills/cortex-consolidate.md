...
Be specific. Not "testing is important" but "mocking at service boundaries instead of HTTP layer caught integration bugs in both web-project-1 and web-project-3."
...
## Build and tooling
- Clear dist/ after any tsconfig change, the build cache doesn't invalidate (web-project-1, api-server)
- Lock file conflicts: delete and regenerate, don't try to merge (web-project-1, frontend)

## Testing
- Mock at the service boundary, not the HTTP layer (api-server, frontend)
- Session-scoped fixtures cause flaky parallel tests (api-server, web-project-3)
```
...
Scanned: web-project-1 (12 learnings), api-server (8 learnings), frontend (5 learnings)
