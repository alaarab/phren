...
async function loadGraphData(client: CortexClient): Promise<GraphPayload> {
  ...
  for (const { projectName, summary, findings, tasks } of perProjectResults) {
    ...
    for (const finding of findings) {
      ...
      const colorMap: Record<string, string> = { decision: "#3b82f6", pitfall: "#ef4444", pattern: "#10b981", other: "#f4a261" };
      nodes.push({
        id: findingId,
        kind: "finding",
        projectName,
...
function classifyFinding(text: string): string {
  ...
  if (lower.includes("pitfall") || lower.includes("gotcha") || lower.includes("warning") || lower.includes("never ") || lower.includes("don't ") || lower.includes("avoid")) return "pitfall";
  if (lower.includes("pattern") || lower.includes("always ") || lower.includes("convention") || lower.includes("standard")) return "pattern";
  return "other";
}
