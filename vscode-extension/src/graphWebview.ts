...
async function fetchProjects(client: CortexClient): Promise<ProjectListItem[]> {
  const raw = await client.listProjects();
  const data = responseData(raw);
  const projects = asArray(data?.projects);
  const parsed: ProjectListItem[] = [];
...
function resolveProjectType(files: ProjectSummaryFile[]): string {
