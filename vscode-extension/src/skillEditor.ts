  ...
  showPreview({
    ...
    onMessage: async (msg: Record<string, unknown>) => {
      if (msg.type === "save" && typeof msg.content === "string") {
        try {
          await client.writeSkill(skillName, msg.content as string, skillSource);
...
function renderSkillEditorHtml(name: string, source: string, content: string, enabled: boolean): string {
