#!/bin/bash
# Link phren knowledge into Claude's config based on the current machine's profile.
#
...
MACHINE_FILE="$HOME/.phren/.machine-id"
CONTEXT_FILE="$HOME/.phren-context.md"

# Allow overriding machines.yaml location for personal (non-framework) use
MACHINES_FILE="${PHREN_PERSONAL:-$SCRIPT_DIR/machines.yaml}"
...
register_machine() {
  ...
  echo "This machine isn't registered with phren yet."
  echo ""
...
# outside the <!-- phren-managed --> markers.
write_context_file() {
  local managed_content="$1"
  local wrapped=""
  wrapped+="<!-- phren-managed -->"$'\n'
  wrapped+="$managed_content"$'\n'
  wrapped+="<!-- phren-managed -->"

  if [ -f "$CONTEXT_FILE" ] && grep -q '<!-- phren-managed -->' "$CONTEXT_FILE" 2>/dev/null; then
    # Preserve user content before and after the managed block
    local before after
    before="$(sed -n '1,/<!-- phren-managed -->/{ /<!-- phren-managed -->/d; p; }' "$CONTEXT_FILE")"
    after="$(sed -n '/<!-- \phren-managed -->/,${  /<!-- \phren-managed -->/d; p; }' "$CONTEXT_FILE")"
...
rebuild_memory() {
  ...
  if [ -f "$memory_file" ]; then
    header="$(sed '/^<!-- phren:projects:start -->/,$d' "$memory_file")"
  fi

  # Build a pointer table (not inline content) for token efficiency
  local managed=""
  managed+="<!-- phren:projects:start -->"$'\n'
...
configure_mcp() {
  ...
  for candidate in \
    ...
    "$HOMEphren/mcp/dist/index.js" \
    "$HOME/Projectsphren/mcp/dist/index.js"; do
    if [ -f "$candidate" ]; then
      mcp_dist="$candidate"
      mcp_cmd="node"
      mcp_args="$candidate"
  ...
  # Check if phren MCP entry already exists
  if grep -q '"phren"' "$settings_file" 2>/dev/null; then
  ...
  if command -v jq &>/dev/null; then
    ...
    jq --arg cmd "$mcp_cmd" --arg dist "${mcp_dist:-}" --arg dir "$SCRIPT_DIR" \
      ".mcpServers.phren = {\"command\": \$cmd, \"args\": $jq_args_expr}" \
    ...
    echo ""
    echo "  \"mcpServers\": {"
    echo "    \"phren\": {\"command\": \"$mcp_cmd\", \"args\": [$(echo "$mcp_args" | sed 's/ /", "/g; s/^/"/; s/$/"/' )]}"
    echo "  }"
    echo ""
    echo "no_jq"
    return 0
...
configure_vscode_mcp() {
  ...
  if [ -f "$mcp_file" ] && grep -q '"phren"' "$mcp_file" 2>/dev/null; then
    echo "already_configured"
    return 0
  fi
  ...
  if command -v jq &>/dev/null; then
    ...
    if [ -f "$mcp_file" ]; then
      jq --arg cmd "$mcp_cmd" --arg dist "${mcp_dist:-}" --arg dir "$SCRIPT_DIR" \
        ".servers.phren = {\"command\": \$cmd, \"args\": $jq_args_expr}" \
        "$mcp_file" > "$tmp_file"
    else
      jq -n --arg cmd "$mcp_cmd" --arg dist "${mcp_dist:-}" --arg dir "$SCRIPT_DIR" \
        "{\"servers\": {\"phren\": {\"command\": \$cmd, \"args\": $jq_args_expr}}}" > "$tmp_file"
    fi
    mv "$tmp_file" "$mcp_file"
  else
    if [ ! -f "$mcp_file" ]; then
      if [ "$mcp_cmd" = "npx" ]; then
        printf '{\n  "servers": {\n    "phren": {"command": "npx", "args": ["-y", "phren", "%s"]}\n  }\n}\n' "$SCRIPT_DIR" > "$mcp_file"
      else
        printf '{\n  "servers": {\n    "phren": {"command": "node", "args": ["%s"]}\n  }\n}\n' "$mcp_dist" > "$mcp_file"
      fi
    else
      echo "no_jq"
      return 0
...
main() {
  ...
  case "$mcp_status" in
    installed)          echo "  Claude: installed phren MCP server" ;;
    already_configured) echo "  Claude: phren MCP already configured" ;;
  ...
  case "$vscode_status" in
    installed)          echo "  VS Code: installed phren MCP server" ;;
    already_configured) echo "  VS Code: phren MCP already configured" ;;
