...
import { authProfilesPath, getAuthStatusEntries, removeApiKeyProfile, upsertApiKeyProfile, } from "@phren/cli/auth/profiles";
function parseApiKeyProvider(raw) {
    if (raw === "openai" || raw === "openrouter" || raw === "anthropic")
        return raw;
    return null;
}
function envVarForApiProvider(provider) {
    switch (provider) {
        case "openai":
...
export async function runAgentCli(raw) {
    ...
    const modelName = provider.model ?? args.model ?? provider.name;
        ...
        if (args.resume && phrenCtx) {
            ...
            if (priorSnapshot && priorSnapshot.messages.length > 0) {
                if (args.verbose) {
                    ...
                    process.stderr.write(`Resuming session ${priorSnapshot.sessionId.slice(0, 8)} (${projectLabel}) with ${priorSnapshot.messages.length} messages\n`);
                }
                const contextLimit = provider.contextWindow ?? 200_000;
                const session = createSession(contextLimit);
                session.messages = priorSnapshot.messages;
