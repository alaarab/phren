...
function colorize(line, rules, resetSeq = RESET) {
...
function highlightTSCode(line, colors) {
    const rst = colors?.reset ?? RESET;
...
function highlightPYCode(line, colors) {
    const rst = colors?.reset ?? RESET;
    return colorize(line, [
        [/""".*?"""|'''.*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'/g, colors?.string ?? GREEN], // strings
        [/\b\d+(\.\d+)?\b/g, colors?.number ?? YELLOW], // numbers
...
const bashHighlight = (line, colors) => {
    const commentColor = colors?.comment ?? GRAY;
    const rst = colors?.reset ?? RESET;
    // Comment
    const hashIdx = line.indexOf("#");
...
function highlightBashCode(line, colors) {
    const rst = colors?.reset ?? RESET;
    return colorize(line, [
        [/"(?:[^"\\]|\\.)*"|'[^']*'/g, colors?.string ?? GREEN], // strings
...
// ── JSON ────────────────────────────────────────────────────────────
const jsonHighlight = (line, colors) => {
    const rst = colors?.reset ?? RESET;
    return colorize(line, [
...
// ── CSS / SCSS ──────────────────────────────────────────────────────
const cssHighlight = (line, colors) => {
    const commentColor = colors?.comment ?? GRAY;
...
// ── Generic fallback ────────────────────────────────────────────────
const genericHighlight = (line, colors) => {
    const rst = colors?.reset ?? RESET;
    return colorize(line, [
        [/"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'/g, colors?.string ?? GREEN], // strings
 ...
 */
export function highlightCode(code, language, colors) {
