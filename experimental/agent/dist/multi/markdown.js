...
export function renderMarkdown(text, colors) {
...
function osc8Link(url, text, linkStyle, reset) {
    return `\x1b]8;;${url}\x07${linkStyle}${text}${reset}\x1b]8;;\x07`;
}
/** Apply inline formatting: bold, italic, inline code, links. */
function renderInline(text, bold = BOLD, italic = ITALIC, code = CYAN + INVERSE, reset = RESET, link = `${ESC}4m${CYAN}`) {
    let result = text;
