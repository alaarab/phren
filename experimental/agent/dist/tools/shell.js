...
export const shellTool = {
    ...
    async execute(input) {
        ...
        if (runInBackground) {
            ...
            const task = { pid: child.pid, outputFile, done: false, exitCode: null };
...
export const taskOutputTool = {
    ...
    async execute(input) {
        ...
        let output = "";
        try {
            output = fs.readFileSync(task.outputFile, "utf-8");
        }
        catch { /* no output yet */ }
...
export const taskStopTool = {
    ...
    async execute(input) {
        ...
        const task = backgroundTasks.get(taskId);
        if (!task)
            return { output: `Task "${taskId}" not found.`, is_error: true };
        if (task.done)
            return { output: `Task "${taskId}" already finished.` };
        try {
