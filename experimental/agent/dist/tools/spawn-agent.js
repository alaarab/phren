export function createSendMessageTool(spawner) {
    return {
...
export function createListAgentsTool(spawner) {
    return {
        name: "list_agents",
...
export function createSpawnAgentTool(spawner) {
    return {
        name: "spawn_agent",
...
**TeamAgent** (background=true or idle=true): Spawns a persistent agent with its own conversation. Shows up in the tab bar. The user can switch to its chat and talk to it directly. Use when the user says "spawn a team", "create agents on idle", "spin up workers".`,
        ...
        async execute(input) {
            ...
            const background = input.background || idle;
            if (!name) {
