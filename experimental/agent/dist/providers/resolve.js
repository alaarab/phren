...
import { getDefaultModel, getDefaultReasoningEffort, lookupMaxOutputTokens, normalizeProviderId, normalizeReasoningEffort, } from "../models.js";
function normalizeProviderSelection(overrideProvider, overrideModel) {
...
export function resolveProvider(overrideProvider, overrideModel, overrideMaxOutput, overrideReasoning) {
    ...
    const resolveLimit = (provider, model) => overrideMaxOutput ?? lookupMaxOutputTokens(model, provider);
    const resolveReasoning = (provider, model) => normalizedReasoning ?? getDefaultReasoningEffort(provider, model);
    ...
    if (explicit === "openai" || (!explicit && openAiKey)) {
        if (!openAiKey)
            throw new Error("OpenAI credentials are required. Set OPENAI_API_KEY or run 'phren auth set-key openai'.");
    ...
    if (explicit === "openrouter" || (!explicit && openRouterKey)) {
        if (!openRouterKey)
            throw new Error("OpenRouter credentials are required. Set OPENROUTER_API_KEY or run 'phren auth set-key openrouter'.");
    ...
    if (explicit === "anthropic" || (!explicit && anthropicKey)) {
        if (!anthropicKey)
            throw new Error("Anthropic credentials are required. Set ANTHROPIC_API_KEY or run 'phren auth set-key anthropic'.");
    ...
    throw new Error(`Unknown provider "${explicit}". Supported: openrouter, anthropic, openai, openai-codex, ollama.\n` +
