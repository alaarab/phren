...
export interface AgentTypeDef {
  ...
  allowedTools?: string[];
  /** If set, these tools are removed */
...
export const BUILTIN_AGENT_TYPES = new Map<string, AgentTypeDef>([
  [
    "explore",
    {
      name: "explore",
      ...
      systemPromptPrefix:
        "You are a read-only exploration agent. You can search and read code but cannot modify files.",
    },
  ],
  [
    "plan",
    {
      name: "plan",
      description:
        "Research and planning agent that can read code and capture findings.",
      allowedTools: [
        "read_file",
        "glob",
        "grep",
      ...
      systemPromptPrefix:
        "You are a planning agent. Research the codebase and create a detailed implementation plan.",
    },
  ],
  [
    "general",
 ...
 */
export function loadCustomAgentTypes(
  dir?: string,
): Map<string, AgentTypeDef> {
  const merged = new Map(BUILTIN_AGENT_TYPES);
  const agentDir = dir ?? DEFAULT_AGENT_TYPES_DIR;

  if (!existsSync(agentDir)) return merged;
 ...
 */
function parseFrontmatter(content: string): AgentTypeDef | null {
  ...
  const def: AgentTypeDef = {
    name: raw["name"],
    description: raw["description"] ?? "",
  };

  if (raw["model"]) def.model = raw["model"];
  if (raw["maxTurns"]) def.maxTurns = parseInt(raw["maxTurns"], 10);
  if (raw["allowedTools"]) {
 ...
 */
export function getAgentType(name: string): AgentTypeDef | null {
