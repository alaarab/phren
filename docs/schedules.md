...
| `/v1/schedules/history` | Returns newest-first local runs. Accepts optional `project`, `id`, and `limit` (default 50, maximum 500). Each row carries its status and, when the run was recorded blocked at startup, the `blockedStartupPrompt` text the pane showed. |
