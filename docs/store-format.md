...
⚠️ Known gaps in the current suite, each of which corresponds to a rough edge
above and none of which is currently covered:
...
⚠️ Conformance testing is currently **one-directional** — it proves the Swift
port reads what TypeScript writes, not that TypeScript reads what Swift writes.

⚠️ `generate-fixtures.mjs` is not reproducible for findings, because the finding
write path uses non-injectable `crypto.randomBytes` and `new Date()`. Until
those accept injection, regenerate-and-diff cannot be a CI gate.
