...
Phren already follows this pattern with 59 MCP tools. The key insight: tools should be JSON-in/JSON-out with clear descriptions. The model selects tools based on descriptions alone. Keep tool count manageable or use deferred loading.
...
The SDK is the most direct path to building phren-agent. Key insight: phren already has 59 MCP tools. An agent built with the SDK could use phren as an MCP server, gaining all knowledge operations while the SDK provides the loop, context management, and tool execution.
...
2. **Phren as MCP server**: The agent uses phren's 59 MCP tools for knowledge operations (search, findings, tasks, sessions) while the SDK provides file/shell/web tools.
