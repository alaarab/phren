...
describe.sequential("1.10.x release hardening gates", () => {
  ...
  it("keeps package and MCP server versions consistent", () => {
    ...
    const indexTs = fs.readFileSync(path.join(srcDir, "index.ts"), "utf8");
    const metadataTs = fs.readFileSync(path.join(srcDir, "package-metadata.ts"), "utf8");
