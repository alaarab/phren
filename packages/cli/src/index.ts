...
async function main() {
  // Lazy-load the MCP-server-only module graph (see the note at the top of this
  // file). Only the long-lived MCP server reaches this point, so paying the
  // import cost here — instead of at module scope — keeps every hook subprocess fast.
  const [
    { McpServer },
    { StdioServerTransport },
    { buildIndex, updateFileInIndex: updateFileInIndexFn },
    { runCustomHooks },
    { mcpResponse },
  ...
  // Register all tool handlers from domain modules (lazy-imported: only the MCP
  // server needs them). Registration order is irrelevant — each module registers
  // a disjoint set of tools.
