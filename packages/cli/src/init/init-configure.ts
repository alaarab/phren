...
import { resolveManagementCapabilities, getManagementPreset, type ManagementCapabilities, type ManagementPreset } from "./management-preset.js";
...
export function configureMcpTargets(
  phrenPath: string,
  opts: { mcpEnabled: boolean; hooksEnabled: boolean; caps?: ManagementCapabilities },
  ...
  // "CLI wrapper" line — this early install is intentionally silent. Presets
  // that opt out of wrappers (assisted/manual) skip it; hooks then use the
  // node/npx invocation form.
  if (caps.installWrappers) {
 ...
 * @param verb - label used in log messages, e.g. "Updated" or "Configured"
 */
export function configureHooksIfEnabled(
  phrenPath: string,
  hooksEnabled: boolean,
  verb: string,
  caps?: ManagementCapabilities,
): void {
  const resolvedCaps = caps ?? resolveManagementCapabilities(phrenPath);
  if (hooksEnabled) {
    try {
      const hooked = configureAllHooks(phrenPath, { allTools: true, installWrappers: resolvedCaps.installWrappers });
  ...
  // Presets that opt out of wrappers (assisted/manual) leave the user's PATH and
  // ~/.local/bin untouched; hooks fall back to the node/npx invocation form.
  if (!resolvedCaps.installWrappers) {
    log(`  CLI wrapper: skipped (${getManagementPreset(phrenPath)} preset) — use 'npx @phren/cli <command>' or a global install`);
    return;
  }
...
export function writeWalkthroughEnvDefaults(
  phrenPath: string,
  opts: InitOptions,
  presetContext?: { preset: ManagementPreset; explicit: boolean },
): string[] {
  ...
  const manualPreset = presetContext?.preset === "manual";
  const presetExplicit = presetContext?.explicit === true;
  ...
  if (manualPreset) {
    // Manual preset turns off the prompt-time/background automations. Force the
    // flags to 0 only when the preset was explicitly chosen this run; on a plain
    // re-init we respect any lines the user already set.
    for (const key of ["PHREN_FEATURE_AUTO_CAPTURE", "PHREN_FEATURE_AUTO_EXTRACT", "PHREN_FEATURE_DAILY_MAINTENANCE"]) {
      const present = new RegExp(`^\\s*${key}=.*$`, "m").test(envContent);
      if (presetExplicit || !present) {
        envFlags.push({ flag: `${key}=0`, label: `${key.replace("PHREN_FEATURE_", "").toLowerCase().replace(/_/g, " ")} disabled (manual preset)` });
      }
    }
  } else if (typeof autoCaptureChoice === "boolean") {
    envFlags.push({
