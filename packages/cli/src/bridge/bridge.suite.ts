...
describeAll.skipIf(process.platform === "win32")("standalone Phren service", () => {
...
`);
    ...
    it("times a held AskUserQuestion out into question cards and answers it with digits, Tab and Enter", async () => {
      ...
      // (An approval answered just before, in another case, can still ride the first frame.)
      await waitFor(() => frames.some(f => f.agentStatus?.pendingApproval?.toolName === "AskUserQuestion"), 3_000);
      expect(frames.find(f => f.agentStatus?.pendingApproval?.toolName === "AskUserQuestion")).toBeDefined();
      expect(await reply).toEqual({});
      // Only this question's prompt: a Bash approval answered in an earlier case
      // may still be the pane's remembered prompt in the first frames.
      const prompt = async (after: number) => {
        for (let i = 0; i < 200; i++) {
          for (let index = frames.length - 1; index >= after; index--) if (frames[index].agentStatus?.terminalPrompt?.toolName === "AskUserQuestion") return frames[index].agentStatus.terminalPrompt;
          await sleep(25);
        }
        return undefined;
      };
    ...
    it("does not emit incomplete rows and resets a truncated transcript", async () => {
      // A file of its own: the byte index is shared in this process, and a case
      // that rewrote `record` shorter would make this rewrite look like an append.
      const record = path.join(root, "codex/sessions/2026/09/10/rollout-2026-09-10T00-00-01-truncated.jsonl");
      await writeFile(record, JSON.stringify({ type: "session_meta", payload: { id: session } }) + "\n" + JSON.stringify(row("First message")) + "\n");
