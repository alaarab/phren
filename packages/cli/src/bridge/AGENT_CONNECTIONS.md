...
| `WS /v1/overview?watchApprovals=1` | The `/v1/workspaces` overview for one Herdr server (`server` or `mux`), pushed instead of polled. The first frame is `{type: "overview", ...}` from a fresh snapshot; after that the Hook reads the shared snapshot every `PHREN_OVERVIEW_TICK_MS` (default 2.5 s, the chat streams' share window), rebuilds the overview when that snapshot changed or `PHREN_OVERVIEW_REFRESH_MS` (10 s) passed, and sends it only when its rows differ from the last frame. With nothing to send, `{type: "heartbeat", phren}` every `PHREN_OVERVIEW_HEARTBEAT_MS` (20 s) keeps the phone's copy current. `watchApprovals=1` renews the approval lease each tick. Advertised as `capabilities.overviewStream`; see `server-overview.ts`. |
