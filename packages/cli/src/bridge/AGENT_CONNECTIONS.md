...
| `GET /v1/code/status`, `/v1/code/search`, `/v1/code/outline`, `/v1/code/definition`, `/v1/code/references`, `/v1/code/usage` | Read-only symbol index for the phone's Code screen, present only while the `code` module is on. `status` reports counts, languages, kinds, last index time and top symbols; `search` takes `q`, optional `kind` and `limit`; `outline` takes a project-relative `path`; `definition` and `references` take a `symbol` (`Foo`, `Foo.bar` or `bar()`); `usage` takes `top`. A project with no index is 404 naming `phren code index`. The Hook also re-indexes a project after the git module records a file change (debounced 500 ms), full after a HEAD move. |
