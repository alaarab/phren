...
describe("embedding cache helpers", () => {
  ...
  it("getCachedEmbeddings returns cached result on second call", async () => {
    ...
    const first = await getCachedEmbeddings("/tmpphren", ["hello"], "key", "model");
    const second = await getCachedEmbeddings("/tmpphren", ["hello"], "key", "model");
  ...
  it("embedding errors return empty results instead of throwing", async () => {
    ...
    await expect(getCachedEmbedding("/tmpphren", "hello", "key", "model")).resolves.toEqual([]);
    await expect(getCachedEmbeddings("/tmpphren", ["hello", "world"], "key", "model")).resolves.toEqual([[], []]);
  });
  ...
  it("closes the DB after a successful cached operation", async () => {
    ...
    const result = await getCachedEmbedding("/tmpphren", "hello", "key", "model");
  ...
  it("closes the DB even when the API call throws (single)", async () => {
    ...
    await getCachedEmbedding("/tmpphren", "fail", "key", "model");
  ...
  it("closes the DB even when the API call throws (batch)", async () => {
    ...
    await getCachedEmbeddings("/tmpphren", ["a", "b"], "key", "model");

    expect(db.close).toHaveBeenCalledTimes(1);
  });

  it("getCachedEmbeddings returns empty array for empty input", async () => {
    const result = await getCachedEmbeddings("/tmpphren", [], "key", "model");
  ...
  it("getCachedEmbeddings batches uncached texts and preserves order", async () => {
    ...
    const result = await getCachedEmbeddings("/tmpphren", ["a", "b", "c"], "key", "model");
