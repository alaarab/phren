...
export interface MouseEvent {
  ...
  button: "left" | "middle" | "right" | "scrollUp" | "scrollDown" | "none";
...
export function parseMouseEvent(input: string): MouseEvent | null {
  ...
  let type: MouseEvent["type"] = isRelease ? "release" : "press";

  if (baseCode === 0) button = "left";
  else if (baseCode === 1) button = "middle";
  else if (baseCode === 2) button = "right";
  else if (baseCode === 64) button = "scrollUp";
  else if (baseCode === 65) button = "scrollDown";
  else if (baseCode === 32) { button = "left"; type = "move"; }
  else if (baseCode === 33) { button = "middle"; type = "move"; }
  else if (baseCode === 34) { button = "right"; type = "move"; }
  else if (baseCode === 35) { button = "none"; type = "move"; }
