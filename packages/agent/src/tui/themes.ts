...
export interface Theme {
  ...
  // Message roles (Ink colors)
  user: { label: string; color: string; bold?: boolean };
  agent: { label: string; color: string };
  system: { color: string };

  // Tool calls (Ink colors)
  tool: {
    success: string;
    error: string;
    name: string;
  ...
  // Thinking indicator (RGB tuples for sine-wave interpolation)
  thinking: {
    primary: [number, number, number];
    secondary: [number, number, number];
    verb: string;   // Ink color for the verb text
    elapsed: string; // Ink color for the elapsed time
  };

  // Input area (Ink colors)
  input: {
    separator: string;
    prompt: string;
    bashPrompt: string;
  ...
  // Markdown rendering (ANSI escape sequences for raw renderers)
  markdown: {
    heading: string;
    bold: string;
    italic: string;
  ...
  // Syntax highlighting (ANSI escape sequences)
  syntax: {
    keyword: string;
    string: string;
    number: string;
  ...
  // Diff rendering (ANSI escape sequences)
  diff: {
    added: string;
    removed: string;
    context: string;
  ...
  // Steer queue (Ink colors)
  steer: { color: string; icon: string };

  // History search (Ink colors)
  search: { prompt: string; match: string; noMatch: string };
...
export function getTheme(name?: string): Theme {
  ...
  if (bg) {
    ...
    if (bgColor > 8) return LIGHT_THEME;
  }
  return DARK_THEME;
}
