export function buildSystemPrompt(phrenContext: string, priorSummary: string | null): string {
  const parts = [
    ...
    "",
    "## Interaction",
    "- `ask_user` presents structured questions with optional multiple-choice options. Use this instead of outputting questions as text when you need a clear decision.",
    "- `notebook_edit` edits Jupyter notebook cells (replace, insert, delete) without raw JSON manipulation.",
    "",
    "## Scheduling",
    "- `cron_create` schedules recurring prompts (e.g., 'check CI every 5m'). Session-scoped, auto-expires after 3 days.",
    "- `cron_list` shows active scheduled tasks. `cron_delete` cancels them.",
    "",
    "## Self-configuration",
