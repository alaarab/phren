...
| `packages/cli/src/bridge/terminal.ts` | `TerminalProvider`: what the Hook needs from a terminal multiplexer (list panes, processes, read screen, send keys, prompt, create, start an agent, focus, rename/close, ping). Identity and status stay out of it except as optional hints. `terminalProvider()` returns the active one; tests swap in a fake with `setTerminalProvider`. |
| `packages/cli/src/bridge/terminal-herdr.ts` | The Herdr provider: one call on Herdr's socket per method, through `herdr.ts`'s exported `rpc` (the seam bridge tests fake). Herdr's `agent_session` / `agent_status` become pane hints. `herdr.ts` still reads snapshots and resolves identity directly. |
