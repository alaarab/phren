...
| `packages/cli/src/bridge/terminal-tmux.ts` | The tmux provider, used when no Herdr server answers: the owner's default server (`tmux`) and the Hook's hidden `tmux -L phren` (`tmux-phren`) for phone launches. `list-panes -a` + `ps` become the Hook's snapshot (agent from the foreground command line), `capture-pane`, `send-keys`, `load-buffer`/`paste-buffer`, `respawn-pane` under a login shell. execFile with argument arrays only; `setTmuxDeps` is the test seam, `PHREN_TMUX=off` turns it off (the vitest config does). |
| `packages/cli/src/bridge/pane-status.ts` | Agent status for panes whose terminal does not report one (tmux): the last lifecycle event per pane and terminal instance, in memory and in the pane's binding file. |
...
`@phren/cli` 0.3.4 (see `packages/cli/package.json`).
