...
| `skills/` | Cortex slash commands: sync, init, discover, consolidate, profiles |
...
- `approve_queue_item(id)` : approve an item from the review queue
- `reject_queue_item(id)` : reject an item from the review queue
- `edit_queue_item(id, text)` : edit an item in the review queue before accepting
...
cortex add [path] [--ownership <mode>]            Add current or specified directory as a project
