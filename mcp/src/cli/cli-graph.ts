...
export async function handleGraphLink(args: string[]): Promise<void> {
  ...
  } catch (err: unknown) { if (process.env.PHREN_DEBUG) logger.debug("cli-graph", `graph link insert fragment: ${errorMessage(err)}`); }
  ...
  } catch (err: unknown) { if (process.env.PHREN_DEBUG) logger.debug("cli-graph", `graph link insert document: ${errorMessage(err)}`); }
