...
function ensureDefaultFeatureFlags(phrenPath: string): string[] {
  const created: string[] = [];
...
export function repairPreexistingInstall(phrenPath: string): RepairInstallResult {
...
export function updateMachinesYaml(phrenPath: string, machine?: string, profile?: string) {
  ...
  try {
    ...
    if ((process.env.PHREN_DEBUG || process.env.PHREN_DEBUG)) process.stderr.write(`[phren] updateMachinesYaml parse: ${err instanceof Error ? err.message : String(err)}\n`);
  }
...
export function runPostInitVerify(phrenPath: string): { ok: boolean; checks: PostInitCheck[] } {
  ...
  try {
    ...
    if ((process.env.PHREN_DEBUG || process.env.PHREN_DEBUG)) process.stderr.write(`[phren] runPostInitVerify projectScan: ${err instanceof Error ? err.message : String(err)}\n`);
    ftsOk = false;
  }
  checks.push({
    name: "fts-index",
