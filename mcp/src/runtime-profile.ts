...
export function requestedProfileFromEnv(): string | undefined {
  const profile = process.env.CORTEX_PROFILE?.trim();
 ...
 */
export function resolveRuntimeProfile(cortexPath: string, requestedProfile: string | undefined = requestedProfileFromEnv()): string {
  const result = resolveActiveProfile(cortexPath, requestedProfile);
