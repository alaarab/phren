...
export function cosineFallback(
  ...
  try {
    ...
    logWarn("cosineFallback", `count: ${errorMessage(err)}`);
    return [];
  }
  ...
  try {
    if (totalDocs <= COSINE_CANDIDATE_CAP) {
      ...
      if (safeQ) {
        try {
          ...
          logWarn("cosineFallback", `FTS pre-filter: ${errorMessage(err)}`);
        }
      }
      ...
      if (ftsRows.length < COSINE_CANDIDATE_CAP && totalDocs > 0 && maxRowid >= minRowid) {
        ...
        try {
          ...
          logWarn("cosineFallback", `deterministicSample: ${errorMessage(err)}`);
        }
      }
    ...
    logWarn("cosineFallback", `loadDocs: ${errorMessage(err)}`);
    return [];
  }
...
export async function vectorFallback(
  ...
  if (cache.size() === 0) {
    try { await cache.load(); } catch (err: unknown) {
      logWarn("vectorFallback", `cacheLoad: ${errorMessage(err)}`);
    }
  }
  ...
  return scored.map(e => {
    ...
    try {
      ...
      logWarn("vectorFallback", `fileRead: ${errorMessage(err)}`);
    }
