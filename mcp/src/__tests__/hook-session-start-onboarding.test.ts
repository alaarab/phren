...
describe("hook session start onboarding notice", () => {
  ...
  beforeEach(() => {
    ...
    originalProfile = process.env.PHREN_PROFILE;
  ...
  afterEach(() => {
    if (originalProfile === undefined) delete process.env.PHREN_PROFILE;
    else process.env.PHREN_PROFILE = originalProfile;
    tmp.cleanup();
  });
  ...
  it("returns null once the project is already tracked", () => {
    fs.appendFileSync(path.join(phrenPath, "profiles", "personal.yaml"), "  - repo\n");
    expect(getUntrackedProjectNotice(phrenPath, repoDir)).toBeNull();
  });
  ...
  it("still prompts when the project exists outside the active profile", () => {
    ...
    fs.mkdirSync(path.join(phrenPath, "repo"), { recursive: true });
  ...
  it("still prompts when a same-name tracked project points at a different source path", () => {
    fs.appendFileSync(path.join(phrenPath, "profiles", "personal.yaml"), "  - repo\n");
    const trackedDir = path.join(phrenPath, "repo");
    ...
    fs.writeFileSync(
      path.join(trackedDir, "phren.project.yaml"),
  ...
  it("detects the repo root when the session starts in a subdirectory", () => {
    ...
    const notice = getUntrackedProjectNotice(phrenPath, nestedDir);
    expect(notice).toContain("Ask the user whether they want to add it to phren now.");
  });

  it("returns a first-run onboarding notice when no projects are tracked yet", () => {
    const emptyProfile = path.join(phrenPath, "profiles", "personal.yaml");
  ...
  it("returns a project-seeding notice when a tracked project has no findings/tasks yet", () => {
    fs.appendFileSync(path.join(phrenPath, "profiles", "personal.yaml"), "  - repo\n");
    const trackedProjectDir = path.join(phrenPath, "repo");
    ...
    const notice = getSessionStartOnboardingNotice(phrenPath, repoDir, "repo");
  ...
  it("returns null for tracked projects after memory is captured", () => {
    ...
    expect(getSessionStartOnboardingNotice(phrenPath, repoDir, "repo")).toBeNull();
  });
  ...
  it("returns null once the onboarding marker has already been written", () => {
    ...
    expect(getSessionStartOnboardingNotice(phrenPath, repoDir, null)).toBeNull();
  });
});
