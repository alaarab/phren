...
function touchSentinel(phrenPath: string): void {
  ...
  try {
    ...
    logDebug("touchSentinel", errorMessage(err));
  }
}
  ...
  if (preGlobbed) {
    for (const f of preGlobbed) {
      try {
        ...
        logDebug("computePhrenHash skip", errorMessage(err));
      }
    }
    for (const configPath of topicConfigEntries) {
      try {
        ...
        logDebug("computePhrenHash topicConfig", errorMessage(err));
      }
    }
  } else {
    ...
    for (const dir of projectDirs) {
      ...
      try {
        ...
        logDebug("computePhrenHash globDir", errorMessage(err));
      }
    }
    files.sort();
...
function loadHashMap(phrenPath: string): { version?: number; hashes: Record<string, string> } {
  ...
  try {
    ...
    logDebug("loadHashMap", errorMessage(err));
  }
  return { hashes: {} };
}
  ...
  try {
    ...
    withFileLock(hashFile, () => {
      ...
      try {
        ...
        logDebug("saveHashMap readExisting", errorMessage(err));
      }
...
function readHashSentinel(phrenPath: string): { hash: string; computedAt: number } | null {
  try {
    ...
    logDebug("readHashSentinel", errorMessage(err));
  }
  return null;
}
...
function writeHashSentinel(phrenPath: string, hash: string): void {
  try {
    ...
    logDebug("writeHashSentinel", errorMessage(err));
  }
}
...
function isSentinelFresh(phrenPath: string, sentinel: { computedAt: number }): boolean {
  ...
  for (const dir of dirsToCheck) {
    try {
      ...
      logDebug("isSentinelFresh statDir", errorMessage(err));
    }
  }
  return true;
}
  ...
  try {
    ...
    const anyNewer = allFiles.some(f => {
      try { return fs.statSync(f.fullPath).mtimeMs > graphMtime; } catch (err: unknown) {
        logDebug("loadCachedEntityGraph statFile", errorMessage(err));
        return true;
      }
    });
      ...
      if (Array.isArray(graph.globalEntities)) {
        for (const [entity, project, docKey] of graph.globalEntities) {
          ...
          try {
            ...
            logDebug("loadCachedEntityGraph globalEntitiesInsert2", errorMessage(err));
          }
        }
      } else {
        ...
        try {
          ...
          for (const [name, sourceDoc] of rows) {
            ...
            logDebug("loadCachedEntityGraph globalEntitiesInsert", errorMessage(err));
          }
            }
          }
        } catch (err: unknown) {
          logDebug("entityGraph globalEntitiesRestore", errorMessage(err));
        }
      }
      return true;
    }
  } catch (err: unknown) {
    logDebug("entityGraph cacheLoad", errorMessage(err));
  }
  return false;
}
  ...
  try {
    ...
    for (const link of manualLinks) {
      try {
        ...
        if (!docCheck) {
          logDebug("manualLinks", `pruning stale link to "${link.sourceDoc}"`);
          pruned = true;
          continue;
        }
        ...
        if (projectMatch) {
          try {
            ...
            logDebug("manualLinks globalEntities", errorMessage(err));
          }
        }
      } catch (err: unknown) {
        logDebug("manualLinks entry", errorMessage(err));
      }
    }
    ...
    if (pruned) {
      try {
        ...
        logDebug("manualLinks prune write", errorMessage(err));
      }
    }
  } catch (err: unknown) {
    logDebug("mergeManualLinks", errorMessage(err));
  }
}
...
async function buildIndexImpl(phrenPath: string, profile?: string): Promise<SqlJsDatabase> {
  ...
  try {
    ...
    logDebug("buildIndexImpl userInfo", errorMessage(err));
      ...
      try {
        ...
        for (const entry of allFiles) {
          try {
            ...
            logDebug("buildIndex hashFile", errorMessage(err));
          }
        }
  ...
  logDebug("indexer", `Indexed ${fileCount} files from ${getProjectDirs(phrenPath, profile).length} projects`);
  appendIndexEvent(phrenPath, {
    event: "build_index",
    cache: "miss",
    hash: hash.slice(0, 12),
...
function isRebuildLockHeld(phrenPath: string): boolean {
  ...
  try {
    ...
    logDebug("isRebuildLockHeld stat", errorMessage(err));
    return false;
  }
}
...
async function loadIndexSnapshotOrEmpty(phrenPath: string, profile?: string): Promise<SqlJsDatabase> {
  ...
  try {
    ...
    logDebug("loadIndexSnapshotOrEmpty userInfo", errorMessage(err));
    userSuffix = crypto.createHash("sha1").update(homeDir()).digest("hex").slice(0, 12);
  }
  const cacheDir = path.join(os.tmpdir(), `phren-fts-${userSuffix}`);
...
export function findFtsCacheForPath(phrenPath: string, profile?: string): { exists: boolean; sizeBytes?: number } {
  ...
  try {
    ...
    logDebug("findFtsCacheForPath userInfo", errorMessage(err));
    userSuffix = crypto.createHash("sha1").update(homeDir()).digest("hex").slice(0, 12);
  }
  const cacheDir = path.join(os.tmpdir(), `phren-fts-${userSuffix}`);
  try {
    ...
    logDebug("findFtsCacheForPath", errorMessage(err));
  }
  return { exists: false };
}
