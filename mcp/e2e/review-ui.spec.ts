...
async function openReviewUi(page: Page): Promise<void> {
  await stubExternalAssets(page);
  await page.goto(harness.secureUrl, { waitUntil: "domcontentloaded" });
  await expect(page.locator(".project-card")).toHaveCount(2);
}

test.describe.serial("review-ui browser e2e", () => {
  ...
  test("navigates the major review-ui surfaces from an isolated temp store", async ({ page }) => {
    await openReviewUi(page);
