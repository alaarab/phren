...
export interface ReviewUiHarness {
  rootDir: string;
  homeDir: string;
  cortexDir: string;
  repoADir: string;
...
function seedSkillFixture(cortexDir: string): void {
  writeFile(
    ...
    [
      "---",
      "description: Browser review-ui smoke skill",
      "command: /browser-checks",
      "---",
      "",
      "# Browser E2E Smoke",
      "",
      "Use this skill to validate the browser review-ui smoke flow.",
      "",
    ].join("\n"),
  );
}
...
function seedProjectFixtures(cortexDir: string): void {
  ...
  writeFile(
    ...
    [
      ...
      "- [decision] Review UI should launch from an isolated test store",
      "",
    ].join("\n"),
  );
  writeFile(
...
function startReviewUi(cwd: string, env: NodeJS.ProcessEnv): Promise<StartedReviewUi> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [CLI_PATH, "review-ui", "--no-open", "--port=0"], {
      cwd,
      env,
      stdio: ["ignore", "pipe", "pipe"],
    });
    ...
    const timeout = setTimeout(() => {
      ...
      void stopProcess(child).finally(() => {
        reject(new Error(`Timed out starting review-ui.\nstdout:\n${stdout}\nstderr:\n${stderr}`));
      });
    }, START_TIMEOUT_MS);

    const maybeResolve = () => {
      const publicMatch = stdout.match(/^cortex review-ui running at (.+)$/m);
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null) => {
      cleanupListeners();
      reject(new Error(`review-ui exited early (code=${code}, signal=${signal}).\nstdout:\n${stdout}\nstderr:\n${stderr}`));
    };
...
export async function createReviewUiHarness(): Promise<ReviewUiHarness> {
  ensureCliBuilt();
